...
data class Lens(val label: String, val focus: Int) {
  override fun toString() = "[${this.label} ${this.focus}]"
}

val RGX = Regex("(?<label>\\w+)(?<op>[=-])(?<focus>\\d+)?")
val boxes = Array(256) { mutableListOf<Lens>() }
File("input.txt").readLines()[0].split(',').forEach {
  RGX.matchEntire(it)?.let {
    val label = it.groups["label"]!!.value
    val hash = label.fold(0) { acc, chr -> (acc + chr.toInt()) * 17 % 256 }
    val idx = boxes[hash].indexOfFirst { it.label == label }
    if (it.groups["op"]!!.value[0] == '=') {
      val newLens = Lens(label, it.groups["focus"]!!.value.toInt())
      if (idx >= 0) {
  ...
  // println("After \"$it\":")
  // boxes.withIndex().filter { (_, lenses) -> lenses.size > 0 }.forEach { (idx, lenses) -> println("Box %3d: ".format(idx) + lenses.joinToString(" ")) }
  // println()
}

val result = boxes.withIndex().sumOf { (boxidx, lenses) -> lenses.withIndex().sumOf { (lensidx, lens) -> (boxidx + 1) * (lensidx + 1) * lens.focus } }
println(result)
