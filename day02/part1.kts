...
val GAME_RGX = Regex("^Game (\\d+): ")
val CNT_RGX = Regex("(\\d+) (red|green|blue)")
val result = File("input.txt").readLines().sumBy {
  val gameMatch = GAME_RGX.find(it)
  val valid = it.drop(gameMatch!!.range.endExclusive).splitToSequence("; ").all {
    it.splitToSequence(", ").all {
      val cntMatch = CNT_RGX.matchEntire(it)!!.groupValues
      MAX_CUBES.get(cntMatch[2])!! >= cntMatch[1].toInt()
    }
  }
  if (valid) gameMatch!!.groupValues[1].toInt() else 0
}
println(result)
