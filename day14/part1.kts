...
// The input is 100x100
val WIDTH = 100
val HEIGHT = 100

val lastEmptyRow = IntArray(WIDTH)
val result = File("input.txt").readLines().withIndex().sumOf { (idx, row) ->
  row.withIndex().sumOf { (x, chr) ->
    when (chr) {
      'O' -> (HEIGHT - lastEmptyRow[x]).also { lastEmptyRow[x]++ }
      '#' -> 0.also { lastEmptyRow[x] = idx + 1 }
      else -> 0
    }
  }
}
