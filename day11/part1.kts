...
val rowIdxs = emptyRows.zipWithNext().withIndex().flatMap { (adj, pair) -> (pair.first + adj)..<(pair.second + adj) }
val colIdxs = emptyCols.zipWithNext().withIndex().flatMap { (adj, pair) -> (pair.first + adj)..<(pair.second + adj) }
