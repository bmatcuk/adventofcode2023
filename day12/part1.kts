...
// Function that generates permutations of arrays of length `len` that sum to
// the value of `spaces`.
fun genSpacePermutations(len: Int, spaces: Int) = sequence {
  val perm = IntArray(len)
  suspend fun SequenceScope<IntArray>.generator(idx: Int, spaces: Int) {
    if (idx == 0) {
      perm[0] = spaces
      yield(perm)
    } else if (spaces == 0) {
      idx.downTo(0).forEach { perm[it] = 0 }
      yield(perm)
    } else {
      for (s in 0.rangeTo(spaces)) {
        perm[idx] = s
        generator(idx - 1, spaces - s)
      }
    }
  }
  generator(len - 1, spaces)
}
...
val result = File("input.txt").readLines().sumOf {
  val (records, groups) = it.split(' ').let { (r, g) -> r to g.split(',').map(String::toInt) }
  val recordLen = records.length
  val pattern = records.replace(".", "\\.").replace('?', '.').toRegex()

  // `extraSpaces` is the number of spaces that must exist given a record of
  // length `recordLen` and total group size of `groups.sum()`, minus the
  // mandatory space between each group.
  val extraSpaces = recordLen - groups.sum() - groups.size + 1
  genSpacePermutations(groups.size + 1, extraSpaces)
    .map { it.zip(groups).joinToString(".") { (dots, springs) -> ".".repeat(dots) + "#".repeat(springs) } + ".".repeat(it.last()) }
    .count { it.matches(pattern) }
}
println(result)
