...
val BOUNDING = 200000000000000..400000000000000
...
data class Hail(
  ...
  fun intersects(other: Hail): Boolean {
    val denominator = this.vy * other.vx - this.vx * other.vy
    if (denominator == 0L) {
      return false
    }

    val t1 = (other.vy * (this.x - other.x) - other.vx * (this.y - other.y)) / denominator
    val t2 = (this.vy * (other.x - this.x) - this.vx * (other.y - this.y)) / -denominator
    if (t1 < 0L || t2 < 0L) {
      return false
    }

    val ix = this.x + t1 * this.vx
    val iy = this.y + t1 * this.vy
    return ix in BOUNDING && iy in BOUNDING
  }
}
...
val result = hail.subList(0, hail.size - 1).withIndex()
  .sumOf { (idx, h) -> hail.subList(idx + 1, hail.size).count { h.intersects(it) } }
println(result)
