...
interface Rule {
  val to: String
  fun evaluate(part: Map<Char, Int>): Boolean
}

data class ConditionalRule(val op: Char, val operand1: Char, val operand2: Int, override val to: String) : Rule {
  override fun evaluate(part: Map<Char, Int>): Boolean {
    val value = part[operand1] ?: return false
    return when (op) {
      '<' -> value < operand2
      '>' -> value > operand2
...
data class UnconditionalRule(override val to: String) : Rule {
  override fun evaluate(part: Map<Char, Int>) = true
}
...
tailrec fun runRules(rule: String, part: Map<Char, Int>): Boolean {
  val next = rules[rule]!!.first { it.evaluate(part) }!!.to
  if (next == "A") {
    return true
  } else if (next == "R") {
    return false
  } else {
    return runRules(next, part)
  }
}

val result = lines.drop(blankLineIdx + 1).sumOf {
  val part = it.trim('{', '}').split(',').associate { it[0] to it.drop(2).toInt() }
  if (runRules("in", part)) part.values.sum() else 0
}
println(result)
