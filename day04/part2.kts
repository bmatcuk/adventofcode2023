...
val numWinners = File("input.txt").readLines().map {
  val (winningNumbers, ourNumbers) = it.drop(10).split(" | ").map { it.trimStart().split(Regex("\\s+")).map(String::toInt) }
  winningNumbers.intersect(ourNumbers).size
}
val numCards = MutableList(numWinners.size) { 1 }
for ((idx, winners) in numWinners.withIndex()) {
  for (j in (idx+1)..(idx+winners)) {
    numCards[j] += numCards[idx]
  }
}
val result = numCards.sum()
println(result)
