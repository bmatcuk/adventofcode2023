...
val result = File("input.txt").useLines {
  it.sumOf {
    val card = it.drop(5).take(3).trimStart().toInt()
    try {
      val (winningNumbers, ourNumbers) = it.drop(10).split(" | ").map { it.trimStart().split(Regex("\\s+")).map(String::toInt) }
      val matched = winningNumbers.intersect(ourNumbers)
      if (matched.size == 0) 0UL else 1UL shl (matched.size - 1)
    } catch (e: Exception) {
      println("Failed on card $card")
      throw e
    }
  }
}
