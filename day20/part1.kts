...
for (i in 0..<RUNS) {
  ...
  while (pulses.isNotEmpty()) {
    ...
    // println("$pulse -> ${pulses.size} pulses remain")
    if (pulse.value) {
      highPulses++
    } else {
      lowPulses++
  ...
  counts.add(Counts(lowPulses, highPulses))
  if (modules.values.all { !it.value }) {
    println("cycle detected")
    break
  }
}

var finalCounts = counts.reduce(Counts::plus)
if (counts.size < RUNS) {
  finalCounts = finalCounts * (RUNS / counts.size)
  finalCounts = counts.take(RUNS % counts.size).fold(finalCounts, Counts::plus)
}
println(finalCounts.lowPulses * finalCounts.highPulses)
