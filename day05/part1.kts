...
val maps: MutableMap<String, MutableList<Pair<LongRange, LongRange>>> = hashMapOf()
var currentMap = ""
...
File("input.txt").forEachLine {
  ...
  if (mapLine != null) {
    currentMap = mapLine.groupValues[1]
    mapsTo[currentMap] = mapLine.groupValues[2]
    maps[currentMap] = mutableListOf()
    return@forEachLine
  }

  val (destinationStart, sourceStart, length) = it.split(' ').map(String::toLong)
  maps[currentMap]!!.add((destinationStart..<(destinationStart + length)) to (sourceStart..<(sourceStart + length)))
}

fun categoryToLocation(category: String, num: Long, map: List<Pair<LongRange, LongRange>>): Long {
  val nextCategory = mapsTo[category]!!
  val matchingRanges = map.find { it.second.contains(num) }
  var nextNum: Long
  if (matchingRanges == null) {
    nextNum = num
  } else {
    val (destinationRange, sourceRange) = matchingRanges
    nextNum = destinationRange.start + sourceRange.indexOf(num)
  }

  if (nextCategory == "location") {
    return nextNum
  }
  return categoryToLocation(nextCategory, nextNum, maps[nextCategory]!!)
}

val seedMap = maps["seed"]!!
val minimumLocation = seeds.map { categoryToLocation("seed", it, seedMap) }.min()
